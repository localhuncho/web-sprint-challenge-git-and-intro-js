...
function listOfNames(array) {
  const newArray = array.map((artists) => artists.name);
  return newArray;
}
...
function lotsOfArt(array) {
  const filteredArray = array.filter((artist) => artist.paintings > 100);
  const namesArray = filteredArray.map((artist) => artist.name);
  return namesArray;
}
...
function artistInfo(array, name) {
  ...
  return `no artist found with the name ${name}.`;
}
...
function artistByCountry(array, nationality) {
  const filteredArray = array.filter((artist) => {
    // Check if artist only has one nationality and it matches the desired nationality
    return (
